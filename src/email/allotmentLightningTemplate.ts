/**
 * ============================================================
 * GLCE MUN 2026 — LIGHTNING ROUND ALLOTMENT EMAIL TEMPLATE
 * ============================================================
 *
 * • NO environment variables
 * • EVERYTHING explicitly defined here
 * • Designed to be edited safely by humans
 * • Same visual + structural richness as Priority / First
 *
 * 👉 IMPORTANT:
...
type LightningRoundAllotmentParams = {
  name: string;
  committee: string;
  portfolio: string;
};
...
/* ------------------------------------------------------------
 * Small HTML escape helper (same style as other templates)
 * ------------------------------------------------------------ */
function esc(s: any) {
  return String(s ?? "")
    .replaceAll("&", "&amp;")
...
export function renderLightningRoundAllotmentEmail(p: LightningRoundAllotmentParams) {
  /* ============================================================
  ...
  /* ============================================================
   * 🔴 🔴 🔴 LIGHTNING ROUND — PAYMENT CONFIG (EDIT HERE) 🔴 🔴 🔴
   * ============================================================ */

  // 💰 REGISTRATION FEES — LIGHTNING ROUND
  const REG_FEE_GENERAL = "₹1499"; // 🔴 CHANGE THIS WHEN NEEDED
  const REG_FEE_GLCE = "₹1199"; // 🔴 CHANGE THIS WHEN NEEDED
  const REFUND_AMOUNT = "₹100";
  const MIN_DELEGATION_SIZE = "12";

  // ⏰ PAYMENT DEADLINE — LIGHTNING ROUND
  const PAYMENT_DEADLINE = "11:59 PM on 11th January, 2026"; // 🔴 CHANGE THIS WHEN NEEDED
   ...
   * SUBJECT
   * ============================================================ */
  const subject = `${EVENT_NAME} ${EVENT_YEAR} — Lightning Round Allotment: ${p.committee} (${p.portfolio})`;
...
You have been allotted a portfolio in the LIGHTNING ROUND of ${EVENT_NAME} ${EVENT_YEAR}.
...
REGISTRATION FEE (Lightning Round):
...
<title>${esc(EVENT_NAME)} Lightning Round Allotment</title>
</head>
...
<td style="padding:28px;text-align:center;">
  ...
  <p style="line-height:22px;">
    You have successfully registered for the <b style="color:${GOLD};">Lightning Round</b> of
    <b>${EVENT_NAME} ${EVENT_YEAR}</b>.
    Welcome to the next stage — faster, tighter, and even more intense.
  </p>
  ...
  <p>
    Registration fee for <b>Lightning Round</b> delegates is
